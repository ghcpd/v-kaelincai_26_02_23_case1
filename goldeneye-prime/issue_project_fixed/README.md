# XML Document Parser - XXE Vulnerability Demo (FIXED VARIANT)

> **Note:** This directory (`issue_project_fixed/`) contains the **patched** version of the project. The original vulnerable demo remains in `issue_project/` unchanged for comparison.

## Overview

This is a minimal, runnable project demonstrating an **XXE (XML External Entity) vulnerability** in a document parsing system—and how to fix it. The project simulates an enterprise document management system that processes invoice XML files.

**✅ FIXED:** The parser now rejects DTD/ENTITY declarations and uses `defusedxml` when available.
**⚠️ Original Demo:** The vulnerable code is still available in `../issue_project` for educational purposes.
...
### Expected Results (Fixed Variant)

- **All tests:** ✅ PASS (8 tests total)
- **XXE security tests:** ✅ PASS (blocked)
- **Basic functionality:** ✅ PASS

The parser is now hardened against XXE attacks; malicious XML files are rejected or return safe, empty data.
