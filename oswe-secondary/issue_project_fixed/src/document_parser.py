...
# Use defusedxml to protect against XXE and other XML attacks
...
class DocumentParser:
    ...
    def parse_invoice(self, xml_file_path: str) -> Dict[str, str]:
        ...
        # Attempt to parse securely. defusedxml raises EntitiesForbidden if
        # a document contains dangerous declarations such as external entities.
        try:
            tree = ET.parse(xml_file_path)
            root = tree.getroot()
        except EntitiesForbidden:
            # fallback: sanitize by stripping DOCTYPE and any entity references
            with open(xml_file_path, 'r', encoding='utf-8') as f:
                raw = f.read()
            # remove entire DOCTYPE declaration including internal subset
            raw = re.sub(r'<!DOCTYPE[\s\S]*?\]>','', raw)
            # remove remaining entity references (e.g. &xxe;) to avoid parse errors
            raw = re.sub(r'&[^;]+;','', raw)
            # parse cleaned string; ET.fromstring returns the root element
...
def parse_xml_document(file_path: str) -> Dict[str, str]:
    ...
    This wrapper uses the same secure parser implementation as
    :class:`DocumentParser.parse_invoice` so XXE attacks are blocked.
    """
